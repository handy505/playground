...
def crc16(buf):
    ...
    for ele in buf:
        i = secondByte ^ ele
        secondByte = firstByte ^ table_crc_hi[i]
        firstByte = table_crc_lo[i]
...
if __name__ == '__main__':
	...
	for line in lines:
		...
		lst2 = []
		for ele in lst:
			lst2.append(int(ele, 16)) # convert str to int

		#print("lst2 = {0}".format(lst2))

		for ele in lst2:
			print("{0:02X}".format(ele))
			#print("{0:02X}".format(ele), end=" ")

		ret = crc16(lst2)
		print("crc = 0x{0:02X}".format(ret))
        '''
